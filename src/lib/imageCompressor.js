...
export async function compressImage(file, maxWidth = 1000, maxHeight = 1000, quality = 0.7) {
  // If the file is a HEIC format (common on iPhones), convert it to JPEG client-side first!
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        // to make faint receipt thermal ink extremely dark and paper background pure white
        ctx.filter = 'grayscale(1) contrast(2.3) brightness(1.05)';
