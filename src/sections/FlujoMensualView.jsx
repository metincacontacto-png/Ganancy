      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div>
              <h4 style={{ fontSize: '18px', fontWeight: 700, margin: 0, display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-primary)' }}>
                {title}
                <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-secondary)', backgroundColor: 'var(--border-color)', padding: '2px 8px', borderRadius: '12px', marginLeft: '6px' }}>
                  {items.length}
                </span>
              </h4>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }} onClick={e => e.stopPropagation()}>
            <div style={{ textAlign: 'right' }}>
              <span style={{ fontSize: '12px', color: 'var(--text-secondary)', display: 'block' }}>Total Est.</span>
              <strong style={{ fontSize: '18px' }} className={badgeColorClass}>
                {formatMoney(totalValue)}
              </strong>
            </div>
            <button 
              onClick={() => handleOpenAdd(type, isVariable)} 
              style={{ background: 'var(--accent-light)', border: 'none', color: 'var(--accent)', padding: '8px 16px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13.5px', fontWeight: 600, cursor: 'pointer' }}
            >
        ...
        {isExpanded && (
          <div style={{ padding: '0 0 16px 0' }}>
            {renderTransactionTable(items, type, isVariable ? "Monto Estimado" : "Monto Mensual", `No hay ${title.toLowerCase()} registrados.`)}
          </div>
        )}
      </div>
    );
